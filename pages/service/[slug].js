                   ...
                   {featuredPosts.map((post, index) => (
                     ...
                     <div className="flex-grow ml-4">
                        <p className="text-gray-500 font-xs">{moment(post.createdAt).format('MMM DD, YYYY')}</p>
                           <Link href={`/service/${post.slug}`} className="text-md" key={index} passHref >{post.name}</Link>
                    </div>
                  </div>
                  ))}
               </div>
...
export async function getServerSideProps(context) {
  ...
  await db.connect();
  const post = await Service.findOne({ slug }).lean();
  const featuredPosts = await Service.find({ isFeatured: true }).lean();
